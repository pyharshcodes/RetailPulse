    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3.5">
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-subtle">
          <div className="text-[10px] font-semibold text-slate-500 uppercase">Active Customers</div>
          <div className="text-xl font-bold font-mono text-slate-900 mt-1">{formatNumber(kpis.total_customers)}</div>
          <div className="text-[11px] text-slate-400 mt-1">In active filter period</div>
        </div>

        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-subtle">
          <div className="text-[10px] font-semibold text-slate-500 uppercase">New Customers</div>
          <div className="text-xl font-bold font-mono text-emerald-600 mt-1">{formatNumber(kpis.new_customers)}</div>
          <div className="text-[11px] text-emerald-600 mt-1">First purchase in period</div>
        </div>

        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-subtle">
          <div className="text-[10px] font-semibold text-slate-500 uppercase">Returning Customers</div>
          <div className="text-xl font-bold font-mono text-blue-600 mt-1">{formatNumber(kpis.returning_customers)}</div>
          <div className="text-[11px] text-blue-600 mt-1">Prior purchase history</div>
        </div>

        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-subtle">
          <div className="text-[10px] font-semibold text-slate-500 uppercase">Repeat Purchase Rate</div>
          <div className="text-xl font-bold font-mono text-brand-600 mt-1">{kpis.repeat_purchase_rate}%</div>
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-subtle col-span-2 sm:col-span-1">
          <div className="text-[10px] font-semibold text-slate-500 uppercase">Avg Revenue / Customer</div>
          <div className="text-xl font-bold font-mono text-slate-900 mt-1">{formatINR(kpis.average_revenue_per_customer, false)}</div>
          <div className="text-[11px] text-slate-400 mt-1">ARPU across accounts</div>
        </div>
      </div>

      {/* 2. RFM Customer Segmentation Grid */}
      <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-subtle">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 mb-4 border-b border-slate-100 gap-2">
          <div>
            <h3 className="text-sm font-bold text-slate-900">RFM Behavioral Segmentation</h3>
            <p className="text-xs text-slate-500">
              Recency (days since last purchase), Frequency (orders count), Monetary (total revenue spend).
            </p>
          </div>
          <div className="flex items-center space-x-1.5 text-[11px] text-slate-500 bg-slate-50 px-2.5 py-1 rounded-lg border border-slate-200">
            ...
            <span>Deterministic quartile scoring methodology</span>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-5 gap-3.5">
          {rfm.segments.map(seg => (
            <div key={seg.segment} className="p-4 rounded-xl border border-slate-200 bg-slate-50 flex flex-col justify-between">
              <div>
                ...
                <div className="text-[10px] text-slate-500 mt-0.5">
                  {seg.revenue_share}% of total spend
                </div>
              </div>

              <div className="mt-4 pt-3 border-t border-slate-200/80 text-[10px] text-slate-500 space-y-1">
                <div className="flex justify-between">
                  <span>Avg Orders:</span>
                  <span className="font-mono font-medium text-slate-700">{seg.avg_frequency}</span>
                </div>
                <div className="flex justify-between">
                  <span>Avg Recency:</span>
                  <span className="font-mono font-medium text-slate-700">{seg.avg_recency_days} days</span>
                </div>
                <div className="flex justify-between">
                  <span>Avg Spend:</span>
