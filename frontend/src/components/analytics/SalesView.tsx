    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        return `<div class="font-bold">${item.name}</div>
          <div class="text-slate-300">Revenue: <b>${formatINR(item.value)}</b></div>`;
      }
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-subtle flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h3 className="text-sm font-bold text-slate-900">Interactive Sales Trend</h3>
          <p className="text-xs text-slate-500">Examine network cadence across key metrics and intervals.</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center bg-slate-100 p-1 rounded-lg border border-slate-200 text-xs">
            <button
              ...
              className={`px-2.5 py-1 rounded-md font-medium transition-all ${
                interval === 'monthly' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600'
              }`}
            >
              Monthly
            </button>
            <button
              ...
              className={`px-2.5 py-1 rounded-md font-medium transition-all ${
                interval === 'daily' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600'
              }`}
            >
              Daily
            </button>
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <h4 className="text-sm font-bold text-slate-900 flex items-center">
                <ShoppingBag className="w-4 h-4 mr-1.5 text-brand-600" />
                Sales Channel Distribution
              </h4>
              <p className="text-xs text-slate-500">In-Store vs Assisted Online vs B2B Direct</p>
            </div>
          </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-subtle">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-slate-500">UPI, Net Banking, Credit Cards & B2B Credit Lines</p>
            </div>
          </div>
